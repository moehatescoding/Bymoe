...
export async function generateStaticParams() {
  return ['men','women','kids','home','ikea','our-products'].map(slug => ({ slug }));
}
...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="pt-32 pb-20 max-w-container-max mx-auto px-6 md:px-margin-desktop">
      ...
      {/* Header */}
      <div className="mb-12">
        <h1 className="text-[40px] font-semibold tracking-tight text-primary">{category.name}</h1>
        <p className="text-body-lg text-on-surface-variant mt-2">{category.description}</p>
