...
export default function WorkPage() {
  ...
  return (
    <main className="min-h-screen bg-brand-black flex flex-col justify-center pt-20 sm:pt-32 pb-32 px-4 sm:px-8 md:px-12 max-w-7xl mx-auto overflow-x-hidden relative">
      ...
      <div className="w-full relative z-10 flex flex-col items-center text-center">
        ...
        >
          ...
          <h1 className="font-display text-4xl xs:text-5xl sm:text-7xl md:text-8xl font-black uppercase tracking-tight text-white leading-none mb-4 sm:mb-6">
            PIT STOP <span className="text-[#00ff66]">COLLABORATION</span>
          </h1>
          <p className="text-sm sm:text-xl font-light tracking-tight text-white/70 leading-relaxed max-w-2xl mx-auto">
        ...
        {/* Capabilities Grid */}
        <motion.div 
          className="w-full max-w-5xl mb-16 sm:mb-20 text-left"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            {capabilities.map((cap) => (
              <div 
                key={cap.num}
                className="p-5 sm:p-6 rounded-2xl bg-brand-surface border border-white/10 hover:border-[#00ff66]/40 transition-all duration-300 group"
              >
                <div className="font-mono text-xs text-[#00ff66] mb-2">
                  {cap.num} // SPEC
                </div>
                <h3 className="font-display text-xl sm:text-2xl font-bold uppercase text-white mb-2 group-hover:text-[#00ff66] transition-colors">
                  {cap.title}
                </h3>
        ...
        {/* Action Buttons CTA */}
        <motion.div 
          className="w-full sm:w-auto flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-3 sm:gap-4"
        ...
        >
          <a 
            ...
            className="inline-flex items-center justify-center gap-3 px-6 sm:px-8 py-4 bg-[#00ff66] text-black rounded-xl font-display font-black text-sm uppercase tracking-wider hover:bg-[#22c55e] transition-all shadow-lg shadow-[#00ff66]/20 cursor-pointer text-center"
            data-cursor="EMAIL"
          >
            <span>DISPATCH PROPOSAL // HELLO@BYMOE.IN</span>
            <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <a 
            ...
            className="inline-flex items-center justify-center gap-2.5 px-6 py-4 rounded-xl bg-white/5 border border-white/15 text-white hover:bg-white/10 font-mono text-xs font-bold uppercase tracking-wider transition-all cursor-pointer text-center"
            data-cursor="JOIN"
          >
