...
// ── Floating +1 particle ──
function LikePop({ id, onDone }: { id: number; onDone: () => void }) {
  return (
    <motion.span
      key={id}
      className="absolute -top-5 left-1/2 -translate-x-1/2 text-[10px] font-bold text-pink-400 pointer-events-none select-none"
...
function LikeButton() {
  const STORAGE_KEY = 'bymoe_site_likes';
  ...
  useEffect(() => {
    try {
      const stored = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{"count":0,"liked":false}');
  ...
  function handleLike() {
    const next = { count: count + 1, liked: true };
    setCount(next.count);
    setLiked(true);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(next));
    // Spawn a +1 pop particle
    const id = Date.now();
    setPops((p) => [...p, id]);
  }
  ...
  return (
    <button
      onClick={handleLike}
      className={`relative group ml-3 flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold tracking-widest uppercase transition-all duration-200 cursor-pointer select-none ${
        liked
      ...
      <AnimatePresence>
        {pops.map((id) => (
          <LikePop
            key={id}
            id={id}
            onDone={() => setPops((p) => p.filter((x) => x !== id))}
          />
        ))}
      </AnimatePresence>
    ...
    >
      ...
      <nav className="flex items-center gap-1">
        ...
        <a
          ...
          className="group ml-1 flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold tracking-widest uppercase text-white/40 hover:text-white hover:bg-white/5 transition-all cursor-pointer"
